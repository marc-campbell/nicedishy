...
func ReindexDataFourHour(ctx context.Context, dishyID string, when time.Time) error {
...
from dishy_data where time >= $1 and time < $2 and dishy_id = $3`
	...
	query = `insert into dishy_data_fourhour (time_start, dishy_id, snr, downlink_throughput_bps, uplink_throughput_bps, pop_ping_latency_ms, pop_ping_drop_rate, percent_obstructed, seconds_obstructed)
...
snr = $3, downlink_throughput_bps = $4, uplink_throughput_bps = $5, pop_ping_latency_ms = $6, pop_ping_drop_rate = $7, percent_obstructed = $8, seconds_obstructed = $9`
	...
	if err != nil {
		return fmt.Errorf("error inserting dishy data four hour: %v", err)
	}
...
func ReindexSpeedFourHour(ctx context.Context, dishyID string, when time.Time) error {
	...
	query = `insert into dishy_speed_fourhour (time_start, dishy_id, download_speed, upload_speed) values ($1, $2, $3, $4)
